...
interface IBookingRepository {
  ...
  getMoviesByBookingCount(token: string): Promise<Array<any>>;
}
...
export default class BookingRepository implements IBookingRepository {
  ...
  async getMoviesByBookingCount(token: string): Promise<Array<any>> {
    const res = await axios.get(`${baseURL}/bookings/film/bookings-count/`, {
      headers: getApiHeaders(token)
    });
